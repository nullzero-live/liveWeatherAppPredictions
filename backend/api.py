...
from utils.parameters import ENS_SELECT, CLUSTER_SELECT, COORDINATES_TS, NOW, STARTDATE_TS, ENDDATE_TS, INTERVAL_TS, MODEL, PARAMETERS_TS
from utils.deprecated import deprecated
from utils._constants_ import VERSION, TIME_SERIES_TEMPLATE, GRID_TEMPLATE, POLYGON_TEMPLATE, \
    GRID_TIME_SERIES_TEMPLATE, GRID_PNG_TEMPLATE, LIGHTNING_TEMPLATE, NETCDF_TEMPLATE, STATIONS_LIST_TEMPLATE, \
    INIT_DATE_TEMPLATE, AVAILABLE_TIME_RANGES_TEMPLATE, NA_VALUES, API_BASE_URL, LOGGERNAME, VERSION 
from utils.binary_parser import BinaryParser
from utils.binary_reader import BinaryReader
from utils.exceptions import API_EXCEPTIONS, WeatherApiException
from utils.parsing_util import all_entries_postal, build_coordinates_str_for_polygon, build_coordinates_str, \
...
_logger = setup_logger(LOGGERNAME)
...
def create_path(_file):
    ...
    if not os.path.exists(_path) and len(_path) > 0:
        _logger.info("Create Path: {}".format(_path), exc_info=True)
        os.makedirs(_path)
...
def convert_time_series_binary_response_to_df(bin_input, coordinate_list, parameters, station=False,
    ...
    _logger.info("binary parser complete")
...
def query_station_list(username, password, source=None, parameters=None, startdate=None, enddate=None, location=None,
                       api_base_url=API_BASE_URL, request_type='GET', elevation=None, id=None):
...
def query_station_timeseries(startdate, enddate, interval, parameters, username, password, API_BASE_URL, model='mix-obs',
    ...
    url = TIME_SERIES_TEMPLATE.format(
        ...
        interval=isodate.duration_isostring(interval),
    ...
    return convert_time_series_binary_response_to_df(response.content, coordinates_list, extended_params,
                                                     station=True, na_values=na_values)


def query_special_locations_timeseries(startdate, enddate, interval, parameters, username, password, api_base_url, model='mix',
                                       postal_codes=None, temporal_interpolation=None, spatial_interpolation=None,
                                       on_invalid=None, request_type='GET',
                                       na_values=NA_VALUES):
    """Retrieve a time series from the Meteomatics Weather API.
    ...
    url = TIME_SERIES_TEMPLATE.format(
        ...
        STARTDATE_TS=startdate.isoformat(),
        ENDDATE_TS=enddate.isoformat(),
        INTERVAL_TS=isodate.duration_isoformat(interval),
        PARAMETERS=",".join(parameters),
...
def query_time_series(coordinate_list, startdate:datetime, enddate:datetime, interval, parameters, username, password, api_base_url=API_BASE_URL, model=None,
                      ens_select=None, interp_select=None, on_invalid=None,
                      request_type='GET', cluster_select=None, na_values=NA_VALUES, **kwargs):
    """Retrieve a time series from the Meteomatics Weather API.
    Start and End dates have to be in UTC.
    Returns a Pandas `DataFrame` with a `DateTimeIndex`.
    request_type is one of 'GET'/'POST'
    ...
    # set time zone info to UTC if necessary
    startdate = sanitize_datetime(startdate).isoformat()
    enddate = sanitize_datetime(enddate).isoformat()
    ...
    url = TIME_SERIES_TEMPLATE.format(
        API_BASE_URL=api_base_url,
        COORDINATES_TS=coordinate_list_str,
        STARTDATE_TS=startdate,
        ENDDATE_TS=enddate,
        INTERVAL_TS=isodate.duration_isoformat(interval),
        PARAMETERS_TS=",".join(parameters),
        urlParams="&".join(["{}={}".format(k, v) for k, v in url_params.items()])
    )
...
def query_grid(startdate, parameter_grid, lat_N, lon_W, lat_S, lon_E, res_lat, res_lon, username, password, model=None,
               ens_select=None, interp_select=None, api_base_url=API_BASE_URL, request_type='GET',
               na_values=NA_VALUES,
               **kwargs):
    ...
    # build URL
    url_params = parse_time_series_params(model=MODEL, ens_select=ENS_SELECT, cluster_select=CLUSTER_SELECT,
...
def query_grid_timeseries(startdate, enddate, interval, parameters, lat_N, lon_W, lat_S, lon_E,
                          ...
                          on_invalid=None, api_base_url=API_BASE_URL, request_type='GET', na_values=NA_VALUES):
...
def query_polygon(latlon_tuple_lists, startdate, enddate, interval, parameters, aggregation, username,
                  ...
                  api_base_url=API_BASE_URL, request_type='GET', cluster_select=None, **kwargs):
...
def query_lightnings(startdate, enddate, lat_N, lon_W, lat_S, lon_E, username, password,
                     api_base_url=API_BASE_URL, request_type='GET', model='mix'):
...
def query_netcdf(filename, startdate, enddate, interval, parameter_netcdf, lat_N, lon_W, lat_S, lon_E, res_lat, res_lon,
                 ...
                 api_base_url=API_BASE_URL, request_type='GET', cluster_select=None):
...
def query_available_time_ranges(parameters, username, password, model, api_base_url=API_BASE_URL):
...
def query_grid_png(filename, startdate, parameter_grid, lat_N, lon_W, lat_S, lon_E, res_lat, res_lon, username,
                   password, model=None, ens_select=None, interp_select=None, api_base_url=API_BASE_URL,
                   request_type='GET'):
...
def query_png_timeseries(prefixpath, startdate, enddate, interval, parameter, lat_N, lon_W, lat_S, lon_E, res_lat,
                         ...
                         api_base_url=API_BASE_URL, request_type='GET'):
