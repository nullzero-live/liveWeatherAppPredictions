...
from utils._constants_ import LOGGERNAME
import inspect

logging = setup_logger(LOGGERNAME)
...
def datenum_to_date(date_num):
    ...
    try:
        ...
        conversion = dt.datetime(1, 1, 1) + dt.timedelta(seconds=total_seconds) - dt.timedelta(days=1)
...
def sanitize_datetime(in_date):
    if not isinstance(in_date, dt.datetime):
        raise TypeError(f'Please use datetime.datetime instead of {type(in_date)}')
    elif in_date.tzinfo is None:
        in_date = in_date.replace(tzinfo=pytz.UTC)
        print(in_date)
    else:
        print("Failed")
    return in_date
